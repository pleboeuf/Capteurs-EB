...
void setup()
{
  ...
  {                          // If there's no accumulated event
...
void VacReadVacuumSensor()
{
  ...
  if (fabs(VacAnalogvalue - prev_VacAnalogvalue) > minVacuumChange)
  {                                                                         // Publish event in case of a change in vacuum
...
bool writeEvent(struct Event thisEvent)
{
  ...
  if (savedEventCount < buffSize)
  { // increment the number of saved events up until buffer length
    savedEventCount++;
...
int replayEvent(String command)
{
  ...
  if (targetSerNo >= 0 && targetGen > 0)
  { // Le numéro recherché doit être plus grand que 0
    // Validation
    ...
    { // et plus petit que le numéro de série courant
      return -1;
    }
    if ((noSerie - targetSerNo) > savedEventCount)
    { // Il y a 250 événement au maximum dans le buffer
